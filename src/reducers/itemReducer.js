...
const initialState = {
  isFetching: false, isFetched: false, item: null, transformed: null, error: null, headers: null, deleted: false
}
...
export default (name = '', itemTransformer = null, itemName = 'item') => {
  return (state = initialState, action) => {
    ...
    if (error) {
      return { ...state, isFetching: false, isFetched: true, error: error, item: null, transformed: null, deleted: false }
    }

    if (result) {
      const transformed = itemTransformer ? itemTransformer(result) : null
      return { ...state, isFetching: false, isFetched: true, deleted: false, [itemName]: result, transformed, headers: result.headers }
    }
  }
}

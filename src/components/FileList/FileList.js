...
export default class FileList extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (nextState.files.length !== this.state.files.length) return true;
    return false;
  }
  ...
  render() {
    ...
    return <div>
      <TreeTable
        ...
        noDataText={isFiltering ? "Current filters didn't match any data" : "There are currently no files for this definition"}
        data={files}
        pivotBy={pathColums}
        columns={columns.concat([{
          ...
          filterMethod: (filter, rows) => filter.value.filterValue ? rows.filter(item => item._original && item._original.license && item._original.license.toLowerCase().includes(filter.value.filterValue.toLowerCase())) : rows,
          filterAll: true
        },
        {
          Header: "Copyrights",
